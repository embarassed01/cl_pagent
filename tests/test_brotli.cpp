...
typedef struct {
    ...
    /* Reporting */
    /* size_t would be large enough, until 4GiB+ files are compressed / decompressed on 32-bit CPUs. */
...
static BROTLI_BOOL ParseInt(const char *s, int low, int high, int *result) {
    ...
    if (i > 1 && s[0] == '0') return BROTLI_FALSE;  // ? 03 wrong
...
static Command ParseParams(Context *params) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        /* C99 5.1.2.2.1: "members argv[0] through argv[argc-1] inclusive shall contain pointers to strings"; NULL and 0-length are not forbidden. */
        ...
        /* Too many options. The expected longest option list is:
            "-q0 -w10 -o f -D d -S b -d -f -k -n -v --", i.e. 16 items in total.
            This check is an additional guard that is never triggered, but provides a guard for future changes.
         */
        ...
        if (arg[1] != '-') {
            ...
            for (j = 1; j < arg_len; ++j) {
                ...
                if (c == 'o') {
                    ...
                    quality_set = ParseInt(argv[i], BROTLI_MIN_QUALITY, BROTLI_MAX_QUALITY, &params->quality);  // 0--11
                    if (!quality_set) {
                    ...
                    lgwin_set = ParseInt(argv[i], 0, BROTLI_MAX_WINDOW_BITS, &params->lgwin);  // 10--24
                    if (!lgwin_set) {
                        fprintf(stderr, "error parsing lgwin value [%s]\n", argv[i]);
                        return COMMAND_INVALID;
                    }
                    if (params->lgwin != 0 && params->lgwin < BROTLI_MIN_WINDOW_BITS) {  // < 10
            ...
            if (strcmp("best", arg) == 0) {
                ...
                if (strncmp("dictionary", arg, key_len) == 0) {
                    ...
                    lgwin_set = ParseInt(value, 0, BROTLI_MAX_WINDOW_BITS, &params->lgwin);  // 24
                    if (!lgwin_set) {
                        fprintf(stderr, "error parsing lgwin value [%s]\n", value);
                        return COMMAND_INVALID;
                    }
                    ...
                    lgwin_set = ParseInt(value, 0, BROTLI_LARGE_MAX_WINDOW_BITS, &params->lgwin);  // 30
                    if (!lgwin_set) {
                        fprintf(stderr, "error parsing lgwin value [%s]\n", value);
                        return COMMAND_INVALID;
                    }
                    ...
                    quality_set = ParseInt(value, BROTLI_MIN_QUALITY, BROTLI_MAX_QUALITY, &params->quality);  // 0--11
                    if (!quality_set) {
                        fprintf(stderr, "error parsing quality value [%s]\n", value);
                        return COMMAND_INVALID;
                    }
    ...
    if (params->reject_uncompressible && params->write_to_stdout) return COMMAND_INVALID;
    if (strchr(params->suffix, '/') || strchr(params->suffix, '\\')) return COMMAND_INVALID;  // 后缀
...
static BROTLI_BOOL OpenOutputFile(const char *output_path, FILE **f, BROTLI_BOOL force) {
    int fd;
    *f = NULL;
    if (!output_path) {
...
/**
 * @brief Copy file times and permissions.
...
/**
 * @brief Result ownership is passed to caller.
 * |* dictionary_size *| is set to resulting buffer size.
 * @return BROTLI_BOOL 
 */
static BROTLI_BOOL ReadDictionary(Context *context, Command command) {
    static const int kMaxDictionarySize = BROTLI_MAX_DISTANCE - BROTLI_MAX_BACKWARD_LIMIT(24);
    FILE *f;
    ...
    if (file_size_64 == -1) {
        fprintf(stderr, "could not get size of dictionary file[%s]", PrintablePath(context->dictionary_path));
        fclose(f);
        return BROTLI_FALSE;
    }
    ...
    if (command == COMMAND_COMPRESS) {
        context->prepared_dictionary = BrotliEncoderPrepareDictionary(BROTLI_SHARED_DICTIONARY_RAW, context->dictionary_size, context->dictionary, BROTLI_MAX_QUALITY, NULL, NULL, NULL);
...
static BROTLI_BOOL CloseFiles(Context *context, BROTLI_BOOL rm_input, BROTLI_BOOL rm_output) {
...
static const size_t kFileBufferSize = 1 << 19;  // 524288 (0x80000) 512KiB
...
/**
 * @brief This method might give the false-negative result.
...
/**
 * @brief Internal: should be used only in Provide-/Flush-Output.
 * 
 * @return BROTLI_BOOL
 */
...
static const char * PrettyDecoderErrorString(BrotliDecoderErrorCode code) {
    /* "_ERROR_domain_" is in only added in newer versions. If CLI is linked against older shared library, return error string as is; result might be a bit confusing, e.g. "RESERVED" instead of "FORMAT_RESERVED" */
...
static BROTLI_BOOL DecompressFiles(Context *context) {
    while (NextFile(context)) {
        ...
        /* This allows decoding "large-window" streams. Though it creates fragmentation (new builds decode streams that old builds don't), it is better from used experience perspective. */
        BrotliDecoderSetParameter(s, BROTLI_DECODER_PARAM_LARGE_WINDOW, 1u);
        if (context->dictionary) {
            BrotliDecoderAttachDictionary(s, BROTLI_SHARED_DICTIONARY_RAW, context->dictionary_size, context->dictionary);
        }
...
static BROTLI_BOOL CompressFile(Context* context, BrotliEncoderState* s) {
    ...
    for (;;) {
        ...
        if (!BrotliEncoderCompressStream(s, is_eof ? BROTLI_OPERATION_FINISH : BROTLI_OPERATION_PROCESS, &context->available_in, &context->next_in, &context->available_out, &context->next_out, NULL)) {
            /* Should detect OOM? */
    ...
    while (NextFile(context)) {
        ...
        if (context->lgwin > 0) {
            ...
            if (context->input_file_length >= 0) {
                ...
                while (BROTLI_MAX_BACKWARD_LIMIT(lgwin) < (uint64_t)context->input_file_length) {
                    lgwin++;
