...
namespace trantor {  // helper class for known string length at compile time
    class T {
    public:
...
void Logger::formatTime() {
    ...
    uint64_t microSec = static_cast<uint64_t>(date_.microSecondsSinceEpoch() - date_.roundSecond().microSecondsSinceEpoch());
    if (now != lastSecond_) {
        lastSecond_ = now;
...
#else
            strncpy_s<sizeof lastTimeString_>(lastTimeString_, date_.toFormattedStringLocal(false).c_str(), sizeof(lastTimeString_) - 1);
#endif
        } else {
#ifndef _MSC_VER
            strncpy(lastTimeString_, date_.toFormattedString(false).c_str(), sizeof(lastTimeString_) - 1);
#else
            strncpy_s<sizeof lastTimeString_>(lastTimeString_, date_.toFormattedString(false).c_str(), sizeof(lastTimeString_) - 1);
#endif
        }
    }
...
Logger::Logger(SourceFile file, int line, LogLevel level) : sourceFile_(file),
      fileLine_(line), level_(std::clamp(level, kTrace, kFatal)) {
    formatTime();
    logStream_ << T(logLevelStr[level_], 7);
#ifdef TRANTOR_SPDLOG_SUPPORT
    spdLogMessageOffset_ = logStream_.bufferLength();
...
Logger::Logger(SourceFile file, int line, LogLevel level, const char *func) : sourceFile_(file), fileLine_(line), level_(std::clamp(level, kTrace, kFatal))
#ifdef TRANTOR_SPDLOG_SUPPORT
      ,
      func_(func)
#endif 
