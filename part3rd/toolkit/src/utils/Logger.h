#ifndef _LOGGER_H_TLKIT__
#define _LOGGER_H_TLKIT__
...
namespace trantor {
    ...
    public:
        ...
        /**
         * @brief Calculate of basename of souce files in compile time.
         * 
         */
        class SourceFile {
        public:
    ...
    protected:
        static void defaultOutputFunction(const char *msg, const uint64_t len) { fwrite(msg, 1, static_cast<size_t>(len), stdout); }
        ...
        &outputFunc_(size_t index) {
            ...
            if (index < outputFuncs.size()) { return outputFuncs[index]; }
            while (index >= outputFuncs.size()) { outputFuncs.emplace_back(outputFunc_()); }
            return outputFuncs[index];
        }

        static std::function<void()> &flushFunc_(size_t index) {
            static std::vector<std::function<void()>> flushFuncs;
            if (index < flushFuncs.size()) { return flushFuncs[index]; }
            while (index >= flushFuncs.size()) { flushFuncs.emplace_back(flushFunc_()); }
    ...
    #define LOG_SYSERR_TO(index) trantor::Logger(__FILE__, __LINE__, true).setIndex(index).stream()
    ...
    #define LOG_COMPACT_WARN_TO(index) trantor::Logger(trantor::Logger::kWarn).setIndex(index).stream()
    #define LOG_COMPACT_ERROR trantor::Logger(trantor::Logger::kError).stream()
    #define LOG_COMPACT_ERROR_TO(index) trantor::Logger(trantor::Logger::kError).setIndex(index).stream()
    #define LOG_COMPACT_FATAL trantor::Logger(trantor::Logger::kFatal).stream()
    #define LOG_COMPACT_FATAL_TO(index) trantor::Logger(trantor::Logger::kFatal).setIndex(index).stream()
    #define LOG_COMPACT_SYSERR trantor::Logger(true).stream()
    #define LOG_COMPACT_SYSERR_TO(index) trantor::Logger(true).setIndex(index).stream()
    ...
    #define LOG_FATAL_IF(cond) \
        ...
        #define DLOG_INFO                                                      \
            TRANTOR_IF_(trantor::Logger::logLevel() <= trantor::Logger::kInfo) \
            trantor::Logger(__FILE__, __LINE__).stream()
        #define DLOG_WARN trantor::Logger(__FILE__, __LINE__, trantor::Logger::kWarn).stream()
        #define DLOG_ERROR trantor::Logger(__FILE__, __LINE__, trantor::Logger::kError).stream()
        #define DLOG_FATAL trantor::Logger(__FILE__, __LINE__, trantor::Logger::kFatal).stream()
        #define DLOG_TRACE_IF(cond)                                                 \
            TRANTOR_IF_((trantor::Logger::logLevel() <= trantor::Logger::kTrace) && \
                        (cond))                                                     \
            trantor::Logger(__FILE__, __LINE__, trantor::Logger::kTrace, __func__)  \
