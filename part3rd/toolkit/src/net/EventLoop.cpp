...
namespace trantor {
    ...
    #endif
        quit();
        // Spin waiting for the loop to exit because this may take some time to complete. We assume the loop thread will *always* exit.
        // If this cannot be guaranteed then one option might be to abort waiting and assert(!looping_) after some delay;
    ...
    // The event loop needs a scope exit, so here's the simplest most limited C++14 scope exit available (from https://stackoverflow.com/a/42506763/3173540)
    // TODO: If this is needed anywhere else, introduce a proper on_exit from, for example, the GSL library
    namespace {
        template <typename F> 
        struct ScopeExit {
    ...
    void EventLoop::loop() {
        ...
        try {  // Scope where the loop flag is set
            ...
            LOG_WARN << "Exception thrown from event loop, rethrowing after running functions on quit: " << e.what();
            loopException = std::current_exception();
        }

        // Run the quit functions even if exceptions were thrown 
        // TODO: if more exceptions are thrown in the quit functions, some are left un-run. Can this be made exception safe?
        Func f;
    ...
    void EventLoop::doRunInLoopFuncs() {
        callingFuncs_ = true;
        {
            ...
            // TODO: The following is exception-unsafe. If one of the funcs throws, the remaining ones will not get run. The simplest fix is to catch any exceptions and rethrow them later, but somehow that seems fishy...
            while (!funcs_.empty()) {
                Func func;
                while (funcs_.dequeue(func)) { func(); }
            }
        }
    }
    ...
    void EventLoop::moveToCurrentThread() {
        ...
        if (t_loopInThisThread) {
            LOG_FATAL << "There is already an EventLoop in this thread, you cannot move another in";
            exit(-1);
        }
