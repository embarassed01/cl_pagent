...
namespace trantor {
    ...
    void EpollPoller::updateChannel(Channel *channel) {
        ...
        if (index == kNew || index == kDeleted) {  // a new one, add with EPOLL_CTL_ADD
        #ifndef NDEBUG 
            int fd = channel->fd();
            if (index == kNew) {
                assert(channels_.find(fd) == channels_.end());
                channels_[fd] = channel;  // if key not exist, a pair with that key is created using default values, which is then returned
            } else {  // index == kDeleted
                assert(channels_.find(fd) != channels_.end());
