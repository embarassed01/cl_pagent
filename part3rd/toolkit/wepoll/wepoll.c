...
#define NT_NTDLL_IMPORT_LIST(X)       \
    ...
    X(NTSTATUS, NTAPI, NtCreateFile, (PHANDLE FileHandle, ACCESS_MASK DesiredAccess, POBJECT_ATTRIBUTES ObjectAttributes, PIO_STATUS_BLOCK IoStatusBlock, PLARGE_INTEGER AllocationSize, ULONG FileAttributes, ULONG ShareAccess, ULONG CreateDisposition, ULONG CreateOptions, PVOID EaBuffer, ULONG EaLength))     \
    X(NTSTATUS, NTAPI, NtCreateKeyedEvent, (PHANDLE KeyedEventHandle, ACCESS_MASK DesiredAccess, POBJECT_ATTRIBUTES ObjectAttributes, ULONG Flags))         \
    X(NTSTATUS, NTAPI, NtDeviceIoControlFile, (HANDLE FileHandle, HANDLE Event, PIO_APC_ROUTINE ApcRoutine, PVOID ApcContext, PIO_STATUS_BLOCK IoStatusBlock, ULONG IoControlCode, PVOID InputBuffer, ULONG InputBufferLength, PVOID OutputBuffer, ULONG OutputBufferLength)) \
...
/**
 * @brief 利用 IOCP模型 解决 非阻塞模式通信。IOCP(Windows); epoll(Linux)
 * 
...
通过类比描述，可以看出，IOCP更先进一些
 * 
 * @param iocp_handle 
 * @param afd_device_handle_out 
 * @return int 
 */
int afd_create_device_handle(HANDLE iocp_handle, HANDLE *afd_device_handle_out) {
    ...
    NTSTATUS status;
    /* By opening `\Device\Afd` without specifying any extended attributes, we'll get a handle that lets us talk to the AFD driver, but that doesn't have an associated endpoint (so it's not a socket). */
    status = NtCreateFile(&afd_device_handle, SYNCHRONIZE, &afd__device_attributes,&iosb, NULL, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, FILE_OPEN, 0, NULL, 0);
    ...
    /*
    FILE_SKIP_COMPLETION_PORT_ON_SUCCESS，如果一个句柄设置了这个标记，在操作立即完成时，IO管理器就不会在句柄关联的完成端口上添加完成通知

    对于某些UDP服务器程序，比如DNS服务器，处理的都是比较小的报文，大多数情况下操作都是可以立即完成的，没有必要下次得到完成端口的通知时再处理

    为了略微提高性能，我们可以使用函数 SetFileCompletionNotificationModes 设置不触发 设备内核对象 (只触发 事件内核对象)
    */
...
int afd_poll(HANDLE afd_device_handle, AFD_POLL_INFO *poll_info, IO_STATUS_BLOCK *io_status_block) {
    NTSTATUS status;
    /* Blocking operation阻塞 is not supported. */
    assert(io_status_block != NULL);  // condition不成立则断言报错
    io_status_block->Status = STATUS_PENDING;
    status = NtDeviceIoControlFile(afd_device_handle, NULL, NULL, io_status_block, io_status_block, IOCTL_AFD_POLL, poll_info, sizeof *poll_info, poll_info, sizeof *poll_info);
    if (status == STATUS_SUCCESS) {
        return 0;
...
int afd_cancel_poll(HANDLE afd_device_handle, IO_STATUS_BLOCK *io_status_block) {
    ...
    /* NtCancelIoFileEx() may return STATUS_NOT_FOUND if the operation completed just before calling NtCancelIoFileEx(). This is not an error. */
...
/*
The reflock is a special kind of lock that normally prevents a chunk of memory from being freed, but does allow the chunk of memory to eventually be released in a coordinated fashion.

Under normal operation, threads increase and decrease the reference count, which are wait-free operations.

Exactly once during the reflock's lifecycle, a thread holding a reference to the lock may "destroy" the lock; this operation blocks until all other threads holding a reference to the lock have dereferenced it. After "destroy" returns, the calling thread may assume that no other threads have a reference to the lock.

Attemmpting to lock or destroy a lock after reflock_unref_and_destroy() has been called is invalid and results in undefined behavior. Therefore the user should use another lock to guarantee that this can't happen.
*/
...
/*
N.B.: the tree functions donot set errno or LastError when the fail. Each of the API funcitons has at most one failure mode. It is up to the caller to set an appropriate error code when necessary.
*/
...
HANDLE epoll_create(int size) {
    if (size <= 0) { return_set_error(NULL, ERROR_INVALID_PARAMETER); }
    return epoll__create();
}

HANDLE epoll_create1(int flags) {
    if (flags != 0) { return_set_error(NULL, ERROR_INVALID_PARAMETER); }
    return epoll__create();
}
...
err:
    /* On Linux, in the case of epoll_ctl(), EBADF(ERROR_INVALID_HANDLE) takes priority over other errors. Wepoll mimics模仿 this behavior. */
    err_check_handle(ephnd);
...
int err_check_handle(HANDLE handle) {
    DWORD flags;
    /* GetHandleInformation() succeeds when passed `INVALID_HANDLE_VALUE`, so check for this condition explicitly. */
...
int init(void) {
    if (!init__done && !InitOnceExecuteOnce(&init__once, init__once_callback, NULL, NULL)) {
        /* 
        `InitOnceExecuteOnce()` itself is infallible, and it doesn't set any error code when the once-callback returns FALSE. We return -1 here to indicate that global initialization failed; the failing init function is resposible for setting `errno` and calling `SetLastError()`. 
        */
        return -1;
    }
    return 0;
...
#define POLL_GROUP__MAX_GROUP_SIZE 32   // 轮询组最大32
...
static poll_group_t * poll_group__new(port_state_t *port_state) {
    ...
    if (poll_group == NULL) return_set_error(NULL, ERROR_NOT_ENOUGH_MEMORY);
...
poll_group_t * poll_group_acquire(port_state_t *port_state) {
    queue_t *poll_group_queue = port_get_poll_group_queue(port_state);
    poll_group_t *poll_group = !queue_is_empty(poll_group_queue) ? container_of(queue_last(poll_group_queue), poll_group_t, queue_node) : NULL;  // 移到队列的最后一个
    if (poll_group == NULL || poll_group->group_size >= POLL_GROUP__MAX_GROUP_SIZE) {
        poll_group = poll_group__new(port_state);  // 轮询组数太多，就新建一个组
    }
    if (poll_group == NULL) {
        return NULL;
    }
    if (++poll_group->group_size == POLL_GROUP__MAX_GROUP_SIZE) {  // 环
...
void poll_group_release(poll_group_t *poll_group) {
    ...
    queue_move_to_end(poll_group_queue, &poll_group->queue_node);  // 移到队列尾部
    /* Poll groups are currently only freed when the epoll port is close. */
}
...
static inline port_state_t * port__alloc(void) {
    ...
    if (port_state == NULL) return_set_error(NULL, ERROR_NOT_ENOUGH_MEMORY);
    return port_state;
}
...
static inline HANDLE port__create_iocp(void) {
    ...
    if (iocp_handle == NULL) return_map_error(NULL);
    return iocp_handle;
}
...
port_state_t * port_new(HANDLE *iocp_handle_out) {
    ...
    if (port_state == NULL) goto err1;
    iocp_handle = port__create_iocp();
    if (iocp_handle == NULL) goto err2;
...
int port_delete(port_state_t *port_state) {
    ...
    /* At this point the IOCP port should have been closed. 
        `port_close()` should be called before `port_delete()`
    */
...
static inline int port__feed_events(port_state_t *port_state, struct epoll_event *epoll_events, OVERLAPPED_ENTRY *iocp_events, DWORD iocp_event_count) {
...
static inline int port__poll(port_state_t *port_state, struct epoll_event *epoll_events, OVERLAPPED_ENTRY *iocp_events, DWORD maxevents, DWORD timeout) {
...
int port_wait(port_state_t *port_state, struct epoll_event *events, int maxevents, int timeout) {
    ...
    /* Decide whether the IOCP completion list can live on the stack, or allocate memory for it on the heap. */
    ...
    /* Compute the timeout for GetQueuedCompletionStatus, and the wait end time, if the user specified a timeout other than zero or infinite. */
    if (timeout > 0) {
    ...
    /* Dequeue completion packets until either at least one interesting event has been discovered, or the timeout is reached. */
    for (;;) {
        uint64_t now;
        result = port__poll(port_state, events, iocp_events, (DWORD)maxevents, gqcs_timeout);
        if (result < 0 || result > 0) break;  /* Result, error, or time-out */
    ...
    if (iocp_events != stack_iocp_events) free(iocp_events);
    if (result >= 0) return result;
    else if (GetLastError() == WAIT_TIMEOUT) return 0;
    else return -1;
}
...
sock_state_t * port_find_socket(port_state_t *port_state, SOCKET socket) {
    ...
    if (tree_node == NULL) return_set_error(NULL, ERROR_NOT_FOUND);
...
void port_request_socket_update(port_state_t *port_state, sock_state_t *sock_state) {
    if (queue_is_enqueued(sock_state_to_queue_node(sock_state))) return;  // 已排队
...
static inline void queue__detach_node(queue_node_t *node) {
    ...
    node->next->prev = node->prev;  // 卸载该节点
}
...
int reflock_global_init(void) {
    ...
    if (status != STATUS_SUCCESS) return_set_error(-1, RtlNtStatusToDosError(status));
    return 0;
}
...
static void reflock__signal_event(void *address) {
    ...
    if (status != STATUS_SUCCESS) abort();
}
...
static inline sock_state_t * sock__alloc(void) {
    ...
    if (sock_state == NULL) return_set_error(NULL, ERROR_NOT_ENOUGH_MEMORY);
    return sock_state;
}
...
sock_state_t * sock_new(port_state_t *port_state, SOCKET socket) {
    ...
    if (socket == 0 || socket == INVALID_SOCKET) return_set_error(NULL, ERROR_INVALID_HANDLE);
    base_socket = ws_get_base_socket(socket);
    if (base_socket == INVALID_SOCKET) return NULL;
    poll_group = poll_group_acquire(port_state);
    if (poll_group == NULL) return NULL;
    sock_state = sock__alloc();
    if (sock_state == NULL) goto err1;
...
static int sock__delete(port_state_t *port_state, sock_state_t *sock_state, bool force) {
    ...
    /* If the poll request still needs to complete, the sock_state object can't be free()d yet. 
    `sock_feed_event()` or `port_close()` will take care of this later. */
...
int sock_set_event(port_state_t *port_state, sock_state_t *sock_state, const struct epoll_event *ev) {
    /* EPOLLERR and EPOLLHUP are always reported, even when not requested by the caller.
    However they are disabled after a event has been reported for a socket for which the EPOLLONESHOT flag was set. */
...
static inline DWORD sock__epoll_events_to_afd_events(uint32_t epoll_events) {
    /* Always monitor for AFD_POLL_LOCAL_CLOSE, which is triggered when the socket is closed with closesocket() or CloseHandle(). */
...
int sock_update(port_state_t *port_state, sock_state_t *sock_state) {
    ...
    if ((sock_state->poll_status == SOCK__POLL_PENDING) && (sock_state->user_events & SOCK__KNOWN_EPOLL_EVENTS & ~sock_state->pending_events) == 0) {
        ...
        if (afd_poll(poll_group_get_afd_device_handle(sock_state->poll_group), &sock_state->poll_info, &sock_state->io_status_block) < 0) {
            switch (GetLastError()) {
                ...
                default:  /* Other errors are propagated to the caller. */
                    return_map_error(-1);
            }
        }
...
int sock_feed_event(port_state_t *port_state, IO_STATUS_BLOCK *io_status_block, struct epoll_event *ev) {
    ...
    /* If the the socket has the EPOLLONESHOT flag set, unmonitor all events, even EPOLLERR and EPOLLHUP. But always keep looking for closed sockets. */
...
int ws_global_init(void) {
    ...
    if (r != 0) return_set_error(-1, (DWORD)r);
    return 0;
}
...
SOCKET ws_get_base_socket(SOCKET socket) {
    ...
    for (;;) {
        ...
        error = GetLastError();
        if (error == WSAENOTSOCK) return_set_error(INVALID_SOCKET, error);
