...
int open_udp_channel(const char *host, int port)
{
    ...
    if (setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, (const char*)&timeout, (int)sizeof(timeout)) != 0)
    {
    ...
    while (RUN)
    {
        ...
        num_bytes_received = recvfrom(sockfd, (unsigned char *)packet_in, MAX_PACKET_SIZE, 0, NULL, NULL);
