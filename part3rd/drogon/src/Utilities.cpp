...
{
    // std::get_time is defined s.t. its format parameters are the exact same as strptime.
...
time_t timegm(struct tm *tm)
{
    ...
    /* _mkgmtime() changes the value of the struct tm* you pass in, so use a copy
    */
...
{
    static const std::string_view char_space = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
...
void binaryStringToHex(const char *ptr,
                        size_t length,
                        char *out,
                        bool lowerCase)
{
...
std::string secureRandomBytes(size_t size)
{
    if (size == 0)
        return std::string();
    ...
    // batch up to 32 bytes of random data for efficiency. Calling secureRandomeBytes can be expensive.
    auto randByte = []() {
